...
public class AppInterface {
	...
	private ListOfObjects[] listOfObjects = new ListOfObjects[3];
	...
	public void chooseSuitableTeachers() {
		...
		while (listOfObjects[0].hasNext()) {
			...
			while (course.getListOfRequirements().hasNext()) {
				...
				String[] tokens = string.trim().split(" ");
				for (String s : tokens) {
					if (!(((Teacher) listOfObjects[1].find(s)).getSkill().equals(requirement.getContent()))) {
						((Teacher) listOfObjects[1].find(s)).getListOfTrainings().add(requirement);
					}
				}
